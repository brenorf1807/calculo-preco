...
**Custo fixo virou percentual sobre o faturamento estimado da empresa, não mais rateio por volume de um produto específico.** O MVP original (seguindo a spec) ratava custo fixo por volume estimado do item sendo calculado — o usuário apontou dois problemas reais: (1) obriga informar volume a cada cálculo, e (2) se aplicado independentemente a cada produto (o padrão de uso real, calcular vários produtos ao longo do tempo), soma o custo fixo inteiro várias vezes, uma por produto, distorcendo tudo. A correção: `Empresa.faturamentoMensalEstimadoReais` (um valor só, cadastrado uma vez, em Empresa ou Custos Fixos) e `CustoFixoMensal.percentualSobreFaturamento()` calculam **um percentual único** (custo fixo total ÷ faturamento estimado), aplicado no denominador da fórmula de precificação junto com tributos e custos variáveis — mesmo tratamento dado aos outros custos percentuais, e o mesmo método que consultores de precificação para pequenos negócios costumam ensinar. Mudança de formato no motor: `PriceCalculatorInput.custoFixoRateadoUnitario: Money` (numerador) virou `percentualCustoFixo: Percentage` (denominador). As funções antigas de rateio por volume/tempo de produção (`rateioPorVolume`, `rateioPorTempoProducao`, `analiseSensibilidadeVolume`) continuam no motor, testadas — só não são mais chamadas pelo `PricingService`; podem ser reaproveitadas numa tela de relatório gerencial futura sem precisar reescrever nada.
