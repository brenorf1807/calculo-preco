...
2. **Único ponto de arredondamento do custo direto**: a camada de orquestração (`calcularPrecoDeVenda` em `packages/api/src/services/calculo.service.ts`) soma matéria-prima + embalagem + mão de obra em `Decimal` e só então chama `Money.fromReais(...)` **uma vez**, exatamente onde esse valor entra na fórmula de precificação.
